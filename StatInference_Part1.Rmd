...
##Simulations 
I will create 1000 random exponential distributions of iid variables, all with rate lambda = 0.2 and # of samples n=40. For each one of these I will be calculating and saving their mean.
...
```{r fig.height=4,echo=TRUE, message=FALSE}
...
```
Distribution **mean= `r round(mean(d),2)`** tends to equal the mean of standard normal distrubution, which is 0. 
Distribution **st. deviation=  `r round(sd(d),2)` ** tends to equal the st. deviation of standard normal distrubution, which is 1.
...
for (i in 1 : 1000) 
  {
    #run simulation of exponential & add to the estimates table
    dist2<- rexp(n2, lambda) ; ens2<-c(ens2, dist2)
    #mean & variance of n samples simulation calculated and stored in the relevant table, aka mns2 and vns2
    mns2 <- c(mns2, mean(dist2)) ; vns2 <- c(vns2, sd(dist2)^2 ) }
# normalized distribution of means
...
abline(v=mean(d2), col="red", lwd= 3); abline(v=sd(d2), col="violet", lty=2, lwd= 1) ; abline(v=-sd(d2), col="violet", lty=2, lwd= 1)
```
The **distribution mean  `r round(mean(d2),2)`** tends to equal the mean of standard normal distrubution, which is 0
The **distribution St deviation `r round(sd(d2),2) ` ** tends to equal the St. Deviation of standard normal distrubution, which is 1
